  ...
  .map((item, index) => (
    <div key={item.title} className="group inline-block text-center">
      ...
      >
        ...
        >
          <Image className="h-10 w-10" alt={item.title} src={item.imgSrc} width={10} height={10} />
        </div>
      </div>
    </div>
  ))
    ...
    <BentoCard linkTo={linkTo} height="h-[276px]">
      ...
      <div className="mt-4 flex items-center justify-center gap-3 transition-all duration-500 ease-in-out md:mt-6">
        ...
        <div className="col-1 row-start-2">
          <h2 className="mb-2 font-medium">Toolbox</h2>
          <p className="text-text-secondary">
            Things I&apos;ve worked with and built on
          </p>
        </div>
    </BentoCard>
  );
